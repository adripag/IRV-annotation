...
def clear_form():
    ...
    fields_to_clear = [
        ...
        # Older workflow keys, kept for safety
        "irv1",
        "irv2",
        "irv3",
        "subject_status",
...
sentence = st.text_area(
    ...
    height=100,
    key=f"sentence_input_{st.session_state.form_counter}"
)

verb = st.text_input(
    "Write the verb + RCLI construction (e.g., 'queixar-se')",
    key=f"verb_input_{st.session_state.form_counter}"
)
...
if refl_test == "Yes":
    ...
    if reciprocal_test == "Yes":
        ...
        passive_test = st.radio(
            "3. Passive-like se: Can the sentence be turned into a passive sentence with a likely/plausible subject?",
            ["Select", "Yes", "No"],
            index=0,
            key="passive_test",
            help=(
